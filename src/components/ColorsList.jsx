...
const ColorsList = () => {
  ...
  return (
    <div className={`max-w-3xl mx-auto p-6 ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'} shadow-md rounded-lg`}>
      ...
      <form onSubmit={handleSubmit} className="mb-6">
        <div className="mb-4">
          ...
          <input
            ...
            required
            className={`shadow appearance-none border rounded w-full py-2 px-3 ${isDarkMode ? 'bg-gray-700 text-white' : 'text-black'} leading-tight focus:outline-none focus:shadow-outline`}
          />
        </div>
        <div className="mb-4">
      ...
      <ul className="space-y-4">
        {colors.map(color => (
          <li key={color.id} className={`border-b pb-4 ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
      ...
      {colorToDelete && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className={`p-6 rounded-lg shadow-lg ${isDarkMode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
            <p className="mb-4">Êtes-vous sûr de vouloir supprimer la couleur : {colorToDelete.name} ?</p>
